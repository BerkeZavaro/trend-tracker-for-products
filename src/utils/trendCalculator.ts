...
export const getPreviousYearMonth = (currentMonth: string): string => {
  console.log('getPreviousYearMonth - input:', currentMonth);
  
  // Handle YYYY-MM format
  if (currentMonth.match(/^\d{4}-\d{2}$/)) {
    const [year, month] = currentMonth.split('-');
    const previousYear = (parseInt(year) - 1).toString();
    const result = `${previousYear}-${month}`;
    console.log('getPreviousYearMonth - result:', result);
    return result;
  }
...
export const calculateYoYChange = (currentRow: any, allProductData: any[]) => {
  console.log('calculateYoYChange - currentRow month:', currentRow.month);
  console.log('calculateYoYChange - allProductData length:', allProductData.length);
  console.log('calculateYoYChange - sample months from allProductData:', allProductData.slice(0, 5).map(row => row.month));
  
  const currentMonth = currentRow.month;
  const previousYearMonth = getPreviousYearMonth(currentMonth);
  
  if (!previousYearMonth) {
    console.log('calculateYoYChange - no previous year month calculated');
    return null;
  }
  
  // Find the same month from previous year
  const previousYearRow = allProductData.find(row => row.month === previousYearMonth);
  console.log('calculateYoYChange - looking for month:', previousYearMonth);
  console.log('calculateYoYChange - found previousYearRow:', previousYearRow ? 'YES' : 'NO');
  
  if (!previousYearRow) {
    console.log('calculateYoYChange - no matching previous year data found');
    return null;
  }
  ...
  console.log('calculateYoYChange - returning result:', result);
  return result;
};
